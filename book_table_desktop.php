<?php
	...
	echo '</td><td><a href="author.php?yearstart=1700&yearend=2016&authorpage=' .$row["fullname"]. '">' .$row["Author_First_Name"]. ' ' .$row["Author"]. '</a></td><td><a href="index.php?totalbooks=500&numtitles=100&order=newscore&yearstart=' .$row["Year"]. '&yearend=' .$row["Year"]. '&gsdata=1&alhdata=1&aldata=1&pdata=1&nbadata=1&nytdata=0.0&startnumber=0&order=newscore">' .$row["Year"]. '</a></td><td>' .number_format($row["newscore"],2);
	...
	if ($id != 1){
		mysqli_data_seek( $fictionResults, 0 );
	}
?>
